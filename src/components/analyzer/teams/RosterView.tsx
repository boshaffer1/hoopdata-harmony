...
const RosterView: React.FC<RosterViewProps> = ({
  ...
  const importESPNRoster = async () => {
    ...
    try {
      ...
      const existingTeam = rosters.find(
        team => team.name.toLowerCase() === selectedTeam.displayName.toLowerCase()
      );
      
      if (existingTeam) {
        toast.error(`Team "${selectedTeam.displayName}" already exists`);
        return;
      }
